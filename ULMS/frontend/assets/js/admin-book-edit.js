...
function guardRole() {
  ...
  if (!token || !user || !['admin', 'librarian'].includes(user.role)) {
    location.href = 'login.html?next=admin-books.html';
  }
}
...
// INIT EDIT PAGE
// =========================================================
export async function initBookEditPage() {
  guardRole();

  // ⚠️ Vẽ lại body, NHỚ có ô price
    ...
    <div class="form-container">
      <form id="editBookForm">
        <div class="form-grid">
          ...
          <!-- 🔥 PRICE FIELD ĐÃ THÊM VÀO TEMPLATE -->
          <div class="form-group">
        ...
        <div class="form-actions">
          <button type="button" class="btn-cancel" onclick="location.href='admin-books.html'">
            Cancel
          </button>
  ...
  if (!id) {
    ...
    setTimeout(() => (location.href = 'admin-books.html'), 2000);
    return;
  }
  ...
  const priceEl = document.getElementById('price'); // ✅ giờ KHÔNG còn null
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      setTimeout(() => (location.href = 'admin-books.html'), 1500);
    } catch (err) {
